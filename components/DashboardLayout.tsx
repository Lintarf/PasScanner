...
import { MenuIcon, ScanIcon, IdCardIcon, CameraIcon, DownloadIcon } from './Icons';
...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children, currentPage, navigateTo }) => {
  ...
  const navItems: { id: Page; label: string; icon: React.ReactNode }[] = [
    { id: 'dashboard', label: 'Dashboard', icon: <IdCardIcon className="w-5 h-5" /> },
    { id: 'scanner', label: 'Scanner', icon: <CameraIcon className="w-5 h-5" /> },
    { id: 'manual-scan', label: 'Manual Scan', icon: <ScanIcon className="w-5 h-5" /> },
    { id: 'export-data', label: 'Export Data', icon: <DownloadIcon className="w-5 h-5" /> },
  ];
