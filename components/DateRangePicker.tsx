    ...
    return (
        <div className="relative" ref={pickerRef}>
            <button
                ...
                className="inline-flex w-full sm:w-auto items-center justify-center gap-x-1 px-2 py-1 border border-gray-600 text-xs font-medium rounded-lg shadow-sm text-gray-200 bg-gray-800 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 focus:ring-offset-gray-900"
            >
                <CalendarIcon className="w-3.5 h-3.5 text-gray-400"/>
                <span className="truncate">{getButtonText()}</span>
            </button>
            {isOpen && (
                <div className="absolute top-full mt-2 right-0 z-10 bg-gray-800 border border-gray-700 rounded-xl shadow-2xl w-64 p-2 flex flex-col">
                    <div className="flex items-center justify-between mb-2">
                        <button onClick={prevMonth} className="p-0.5 rounded-full hover:bg-gray-700 text-gray-300"><ChevronLeftIcon className="w-3.5 h-3.5" /></button>
                        <span className="font-bold text-xs text-white">{displayMonth}</span>
                        <button onClick={nextMonth} className="p-0.5 rounded-full hover:bg-gray-700 text-gray-300"><ChevronRightIcon className="w-3.5 h-3.5" /></button>
                    </div>
                    ...
                    <div className="border-t border-gray-700 mt-2 pt-2 grid grid-cols-2 gap-1.5 text-xs">
                        <button onClick={() => handlePreset(7)} className="p-1 text-center text-gray-300 bg-gray-700/50 hover:bg-gray-700 rounded-md">Last 7 Days</button>
                        <button onClick={() => handlePreset(30)} className="p-1 text-center text-gray-300 bg-gray-700/50 hover:bg-gray-700 rounded-md">Last 30 Days</button>
                        <button onClick={() => { setTempStartDate(null); setTempEndDate(null); }} className="p-1 text-center text-gray-300 bg-gray-700/50 hover:bg-gray-700 rounded-md col-span-2">Clear Selection</button>
                    </div>
                    <div className="flex justify-end gap-1.5 mt-2">
                        <button onClick={handleCancel} className="px-2 py-1 text-xs font-medium text-gray-300 rounded-lg hover:bg-gray-700">Cancel</button>
                        <button onClick={handleApply} className="px-2 py-1 text-xs font-medium bg-red-600 text-white rounded-lg hover:bg-red-700">Apply</button>
                    </div>
                </div>
            )}
        </div>
