...
const ScanChart: React.FC<ScanChartProps> = ({ scans, onAreaSelect, activeArea }) => {
    ...
    return (
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-3 h-full">
            <h3 className="text-sm font-bold text-white">Scans per Area</h3>
            <p className="text-xs text-gray-400 mb-2">Click an area to filter history.</p>
            <div className={`mt-4 ${hasData ? 'space-y-2' : 'flex items-center justify-center h-48'}`}>
                {hasData ? (
                    chartData.map(([area, count]) => {
                        ...
                        return (
                           <button
                                ...
                                className={`grid grid-cols-3 gap-3 items-center w-full text-left rounded-lg p-1.5 transition-all duration-200 ${
                                    isActive ? 'bg-red-900/50 ring-2 ring-red-500' : 'hover:bg-gray-700/50'
                                } ${isOtherActive ? 'opacity-50 hover:opacity-100' : ''}`}
                                aria-pressed={isActive}
                            >
                                <p className="text-xs text-gray-300 col-span-1">{area}</p>
                    ...
                    <p className="text-sm text-gray-500 text-center">No scan data available to display chart.</p>
                )}
            </div>
        </div>
    );
